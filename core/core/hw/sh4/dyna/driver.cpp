...
void Sh4Recompiler::Run()
{
	...
	DEBUG_LOG(DYNAREC, "cntx // fpcb offset: %td // pc offset: %td // pc %08X", (u8*)p_sh4rcb->fpcb - sh4_dyna_rcb,
...
void Sh4Recompiler::Init()
{
	...
	// A re-initialization (renderer restart, another game) must emit the main
	// loop at the very beginning of the freshly prepared code cache. The buffer
	// cursor is a static that survives between sessions, so reset it here.
	codeBuffer.reset(true);
	codeBuffer.reset(false);
