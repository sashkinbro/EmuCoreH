...
Renderer *Renderer::getInstance() {
  static std::unique_ptr<Renderer> sRenderer =
      std::make_unique<Renderer>(ConstructorTag{});
...
void Renderer::setWindow(ANativeWindow *window, int32_t width, int32_t height) {
  mWorkerThread.run([=](ThreadState *threadState) {
    ...
    threadState->surface = eglCreateWindowSurface(
        threadState->display, threadState->config, window, NULL);
...
void Renderer::stop() {
  mWorkerThread.run(
      [=](ThreadState *threadState) { threadState->isStarted = false; });
  mHotPocketThread.run([](HotPocketState *hotPocketState) {
    hotPocketState->isStarted = false;
  });
}

float Renderer::getAverageFps() { return averageFps; }
...
Renderer::ThreadState::ThreadState() {
  ...
  eglChooseConfig(display, configAttributes, supportedConfigs.data(),
  ...
  const auto configIter = std::find_if(supportedConfigs.cbegin(),
                                       supportedConfigs.cend(), configMatches);

  config = (configIter != supportedConfigs.cend()) ? *configIter
                                                   : supportedConfigs[0];
...
bool Renderer::ThreadState::configHasAttribute(EGLConfig config,
  ...
  mHotPocketThread.run([this](HotPocketState *hotPocketState) {
    ...
    for (int i = 1; i < 1000; ++i) {
      ...
      while (value != 1) {
        if (value == 0) {
          ALOGI(
              "This will never run, but hopefully the compiler doesn't notice");
        }
        if (value % 2 == 0) {
          value /= 2;
        } else {
...
// should be called once per draw as this function maintains the time delta
// between calls
...
void Renderer::setWorkload(int load) { mWorkload = load; }
...
void Renderer::draw(ThreadState *threadState) {
  ...
  if (threadState->lastUpdate - std::chrono::steady_clock::now() <= 100ms) {
    deltaSeconds =
        (threadState->lastUpdate - std::chrono::steady_clock::now()).count() /
        1e9f;
  }
  threadState->lastUpdate = std::chrono::steady_clock::now();
  ...
  const float aspectRatio =
      static_cast<float>(threadState->width) / threadState->height;

  const std::vector<Circle> circles = {
      {Circle::Color{0.0f, 1.0f, 1.0f}, 0.1f, threadState->x, 0.0f}};
