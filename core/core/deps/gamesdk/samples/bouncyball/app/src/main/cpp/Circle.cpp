...
std::vector<GLfloat> initializeVertices(int segments) {
  // For a triangle fan, the first vertex is the centre, then the following
  // pairs of points form trangles with it.
...
std::array<GLfloat, 16> getMvpMatrix(GLfloat aspectRatio, GLfloat x,
                                     GLfloat y) {
...
void Circle::draw(float aspectRatio, const std::vector<Circle> &circles,
  ...
  glVertexAttribPointer(static_cast<GLuint>(state.vPositionHandle), 2, GL_FLOAT,
                        GL_FALSE, 0, getVertices(segments).data());
...
// This function needs to be called with the same segments number in the same
// frame
