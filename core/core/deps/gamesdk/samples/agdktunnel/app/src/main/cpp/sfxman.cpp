...
SfxMan::SfxMan() {
  ...
  if (result == oboe::Result::OK) {
    ...
    if (result == oboe::Result::OK) {
      ...
      if (_sampleRate <= MAX_SAMPLES_PER_SEC &&
          _sampleRate != oboe::kUnspecified) {
      ...
      ALOGE("Failed to start audio stream. Error: %s",
            oboe::convertToText(result));
    }
  } else {
    ALOGE("Failed to create audio stream. Error: %s",
          oboe::convertToText(result));
  }
}
...
static int _synth(int frequency, int /*duration*/, float amplitude,
                  int16_t *sample_buf, int samples) {
  int i;
...
oboe::DataCallbackResult SfxMan::onAudioReady(
  ...
  if (_frameCount > 0) {
    ...
    if (numFrames < remainingFrames) {
      memcpy(audioData, &_sample_buf[_frameCursor],
