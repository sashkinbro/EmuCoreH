...
class GameAssetManagerInternals {
 public:
  GameAssetManagerInternals(AAssetManager *assetManager, JavaVM *jvm,
                            jobject nativeActivity);

  ~GameAssetManagerInternals();

  uint64_t GetExternalGameAssetSize(const char *assetName,
  ...
  bool LoadExternalGameAsset(const char *assetName, const uint64_t bufferSize,
                             void *loadBuffer, AssetPackInfo *packInfo);

  bool LoadInternalGameAsset(const char *assetName, const uint64_t bufferSize,
                             void *loadBuffer);

  void ChangeAssetPackStatus(
      AssetPackInfo *packInfo,
  ...
  int GetAssetPackCount() const { return mAssetPackCount; }
  ...
  bool GenerateFullAssetPath(const char *assetName,
                             const AssetPackInfo *packInfo, char *pathBuffer,
                             const size_t bufferSize);

  void SetAssetPackInitialStatus(AssetPackInfo &info);

  // Asset Pack Manager support functions below
  bool GetAssetPackManagerInitialized() const {
    return mAssetPackManagerInitialized;
  }
...
GameAssetManagerInternals::GameAssetManagerInternals(
    AAssetManager *assetManager, JavaVM * /*jvm*/, jobject nativeActivity) {
#if defined(NO_ASSET_PACKS)
  ...
  AssetPackErrorCode assetPackErrorCode =
      AssetPackManager_init(jvm, nativeActivity);
  if (assetPackErrorCode == ASSET_PACK_NO_ERROR) {
    ...
    SetAssetPackErrorStatus(
        assetPackErrorCode, NULL,
  ...
  const AssetPackDefinition *AssetPacks =
      AssetManifest_GetAssetPackDefinitions();
  mAssetPacks.reserve(mAssetPackCount);
  mRequestingMobileDownload = false;

  for (int i = 0; i < mAssetPackCount; ++i) {
    ALOGI("GameAssetManager: Setting up asset pack %s",
          AssetPacks[i].mPackName);
...
uint64_t GameAssetManagerInternals::GetExternalGameAssetSize(
    const char *assetName, AssetPackInfo *packInfo) {
  uint64_t assetSize = 0;
  if (packInfo->mAssetPackBasePath == NULL) {
    // If a parent directory base path was not set, assume this is actually an
    // internal asset
    return GetInternalGameAssetSize(assetName);
  }

  char fullAssetFilePath[MAX_ASSET_PATH_LENGTH];
  if (GenerateFullAssetPath(assetName, packInfo, fullAssetFilePath,
...
uint64_t GameAssetManagerInternals::GetInternalGameAssetSize(
    const char *assetName) {
  size_t assetSize = 0;
  AAsset *asset =
      AAssetManager_open(mAssetManager, assetName, AASSET_MODE_STREAMING);
  if (asset != NULL) {
...
void GameAssetManagerInternals::LoadGameAssetAsync(
    const char *assetName, const uint64_t bufferSize, void *loadBuffer,
    LoadingCompleteCallback callback, AssetPackInfo *packInfo, bool isInternal,
    void *userData) {
  char *assetPath = NULL;
  if (packInfo->mAssetPackBasePath == NULL) {
    // If a parent directory base path was not set, assume this is actually an
  ...
  if (!isInternal) {
    ...
    GenerateFullAssetPath(assetName, packInfo, assetPath,
                          MAX_ASSET_PATH_LENGTH);
  }
  mLoadingThread->StartAssetLoad(assetName, assetPath, bufferSize, loadBuffer,
                                 callback, isInternal, userData);
}

bool GameAssetManagerInternals::LoadExternalGameAsset(const char *assetName,
...
bool GameAssetManagerInternals::LoadInternalGameAsset(const char *assetName,
                                                      const uint64_t bufferSize,
                                                      void *loadBuffer) {
...
AssetPackInfo *GameAssetManagerInternals::GetAssetPackByName(
...
AssetPackInfo *GameAssetManagerInternals::GetAssetPackForAssetName(
    const char *assetName) {
  AssetPackInfo *packInfo = NULL;
  int i = 0;
...
bool GameAssetManagerInternals::GenerateFullAssetPath(
    const char *assetName, const AssetPackInfo *packInfo, char *pathBuffer,
    const size_t /*bufferSize*/) {
  bool generatedPath = false;
  const size_t requiredSize =
      strlen(assetName) + strlen(packInfo->mAssetPackBasePath) + 1;
  ...
  ALOGI("GameAssetManager: full external asset path for %s : %s", assetName,
        pathBuffer);
...
#if !defined(NO_ASSET_PACKS)
bool GameAssetManagerInternals::RequestAssetPackDownload(
    const char *assetPackName) {
  ALOGI("GameAssetManager: RequestAssetPackDownload %s", assetPackName);
  AssetPackErrorCode assetPackErrorCode =
      AssetPackManager_requestDownload(&assetPackName, 1);
  ...
  if (success) {
    ...
    SetAssetPackErrorStatus(assetPackErrorCode,
                            GetAssetPackByName(assetPackName),
                            "GameAssetManager: requestDownload");
  }
  return success;
}

void GameAssetManagerInternals::RequestAssetPackCancelDownload(
    const char *assetPackName) {
  ALOGI("GameAssetManager: RequestAssetPackCancelDownload %s", assetPackName);
  // Request cancellation of the download, this is a request, it is not
  // guaranteed that the download will be canceled.
  AssetPackManager_cancelDownload(&assetPackName, 1);
}

bool GameAssetManagerInternals::RequestAssetPackRemoval(
    const char *assetPackName) {
  ALOGI("GameAssetManager: RequestAssetPackRemoval %s", assetPackName);
  AssetPackErrorCode assetPackErrorCode =
      AssetPackManager_requestRemoval(assetPackName);
  bool success = (assetPackErrorCode == ASSET_PACK_NO_ERROR);
...
void GameAssetManagerInternals::RequestMobileDataDownloads() {
  ...
  AssetPackErrorCode assetPackErrorCode =
      AssetPackManager_showCellularDataConfirmation(mNativeActivity);
...
void GameAssetManagerInternals::UpdateAssetPackBecameAvailable(
    AssetPackInfo *assetPackInfo) {
  ...
  if (assetPackInfo->mAssetPackStatus != GameAssetManager::GAMEASSET_READY) {
    ...
    if (assetPackErrorCode == ASSET_PACK_NO_ERROR) {
      AssetPackStorageMethod storageMethod =
          AssetPackLocation_getStorageMethod(assetPackLocation);
      if (storageMethod == ASSET_PACK_STORAGE_FILES) {
        const char *assetPackPath =
            AssetPackLocation_getAssetsPath(assetPackLocation);
...
void GameAssetManagerInternals::UpdateAssetPackFromDownloadState(
    AssetPackInfo *assetPackInfo, AssetPackDownloadState *downloadState) {
  AssetPackDownloadStatus downloadStatus =
      AssetPackDownloadState_getStatus(downloadState);

  switch (downloadStatus) {
    case ASSET_PACK_UNKNOWN:
      break;
    case ASSET_PACK_DOWNLOAD_PENDING:
      ChangeAssetPackStatus(assetPackInfo,
                            GameAssetManager::GAMEASSET_DOWNLOADING);
    ...
    case ASSET_PACK_DOWNLOADING: {
      ChangeAssetPackStatus(assetPackInfo,
                            GameAssetManager::GAMEASSET_DOWNLOADING);
      uint64_t dlBytes =
          AssetPackDownloadState_getBytesDownloaded(downloadState);
      uint64_t totalBytes =
          AssetPackDownloadState_getTotalBytesToDownload(downloadState);
    ...
    case ASSET_PACK_WAITING_FOR_WIFI:
      ChangeAssetPackStatus(assetPackInfo,
                            GameAssetManager::GAMEASSET_NEEDS_MOBILE_AUTH);
      break;
...
void GameAssetManagerInternals::SetAssetPackErrorStatus(
    const AssetPackErrorCode assetPackErrorCode, AssetPackInfo *assetPackInfo,
    const char *message) {
  ...
  if (assetPackInfo == NULL) {
    ...
    ALOGE("%s failed on asset pack %s with error code %d : %s", message,
          assetPackInfo->mDefinition->mPackName,
          static_cast<int>(assetPackErrorCode), mAssetPackErrorMessage);
  }
}
...
GameAssetManager::GameAssetManager(AAssetManager *assetManager, JavaVM *jvm,
  ...
  mInternals =
      new GameAssetManagerInternals(assetManager, jvm, android_context);
}
  ...
  for (int i = 0; i < mInternals->GetAssetPackCount(); ++i) {
    ...
    if (assetPackInfo != NULL) {
      ...
      if (assetPackInfo->mAssetPackStatus ==
          ...
          assetPackInfo->mAssetPackStatus ==
              GameAssetManager::GAMEASSET_DOWNLOADING ||
          assetPackInfo->mAssetPackStatus ==
              GameAssetManager::GAMEASSET_PENDING_ACTION ||
          assetPackInfo->mAssetPackStatus ==
        ...
        AssetPackErrorCode assetPackErrorCode =
            AssetPackManager_getDownloadState(assetPackName, &downloadState);

        if (assetPackErrorCode == ASSET_PACK_NO_ERROR) {
          // Use the returned download state to update our asset pack info
          mInternals->UpdateAssetPackFromDownloadState(assetPackInfo,
                                                       downloadState);
        } else {
          // If an error is reported, mark the asset pack as being in error and
          // bail on the update process
          mInternals->SetAssetPackErrorStatus(
              assetPackErrorCode, assetPackInfo,
      ...
      if (packInfo->mAssetPackStatus == GameAssetManager::GAMEASSET_READY) {
        switch (packInfo->mDefinition->mPackType) {
          ...
          case GAMEASSET_PACKTYPE_ONDEMAND:
            assetSize =
                mInternals->GetExternalGameAssetSize(assetName, packInfo);
            break;
        }
      }
    }
...
bool GameAssetManager::LoadGameAsset(const char *assetName,
                                     const size_t bufferSize,
                                     void *loadBuffer) {
  bool loadSuccess = false;
...
#if defined NO_ASSET_PACKS
    loadSuccess =
        mInternals->LoadInternalGameAsset(assetName, bufferSize, loadBuffer);
#else
    AssetPackInfo *packInfo = mInternals->GetAssetPackForAssetName(assetName);
    if (packInfo != NULL) {
      if (packInfo->mAssetPackStatus == GameAssetManager::GAMEASSET_READY) {
        switch (packInfo->mDefinition->mPackType) {
          case GAMEASSET_PACKTYPE_INTERNAL:
            loadSuccess = mInternals->LoadInternalGameAsset(
                assetName, bufferSize, loadBuffer);
            break;
          case GAMEASSET_PACKTYPE_FASTFOLLOW:
          case GAMEASSET_PACKTYPE_ONDEMAND:
            loadSuccess = mInternals->LoadExternalGameAsset(
                assetName, bufferSize, loadBuffer, packInfo);
            break;
        }
      }
    }
...
bool GameAssetManager::LoadGameAssetAsync(const char *assetName,
                                          const size_t bufferSize,
                                          void *loadBuffer,
...
#endif
        mInternals->LoadGameAssetAsync(assetName, bufferSize, loadBuffer,
                                       callback, packInfo, isInternal,
                                       userData);
        startSuccess = true;
      }
    }
...
const char *GameAssetManager::GetGameAssetParentPackName(
    const char *assetName) {
...
const char **GameAssetManager::GetGameAssetPackFileList(
    const char *assetPackName, int *fileListSize) {
...
GameAssetManager::GameAssetStatus GameAssetManager::GetGameAssetPackStatus(
    const char *assetPackName) {
...
GameAssetManager::GameAssetPackType GameAssetManager::GetGameAssetPackType(
    const char * /*assetPackName*/) {
  GameAssetManager::GameAssetPackType assetPackType =
      GAMEASSET_PACKTYPE_INTERNAL;
...
void GameAssetManager::RequestDownloadCancellation(
    const char * /*assetPackName*/) {
#if !defined(NO_ASSET_PACKS)
...
GameAssetManager::GameAssetStatus GameAssetManager::GetDownloadStatus(
    const char *assetPackName, float *completionProgress,
    uint64_t *totalPackDownloadSize) {
  GameAssetManager::GameAssetStatus assetStatus = GAMEASSET_NOT_FOUND;
