...
class LoaderScene::TextureLoader {
 private:
  ...
  struct LoadedTextureData {
    LoadedTextureData()
        : textureSize(0), textureData(NULL), textureName(NULL) {}
 ...
 public:
  ...
  int NumberCompetedLoading() const {
    return _totalLoadCount - _remainingLoadCount;
  }
  ...
  bool IsAssetPackInstalled(const char *assetPackName) {
    if (strcmp(assetPackName, GameAssetManifest::EXPANSION_ASSETPACK_NAME) ==
        0) {
      return _on_demand_assets_installed;
    } else if (strcmp(assetPackName, GameAssetManifest::MAIN_ASSETPACK_NAME) ==
               0) {
  ...
  void FindTexturesFromAssetPack(const char *assetPackName) {
    ...
    const char **assetPackFiles = gameAssetManager->GetGameAssetPackFileList(
        assetPackName, &assetPackFileCount);
    if (assetPackFiles != NULL) {
      _totalLoadCount += assetPackFileCount;
      _remainingLoadCount += assetPackFileCount;
      ALOGI("TextureLoader: found %d assets from pack %s", assetPackFileCount,
            assetPackName);
    } else {
      ALOGI("TextureLoader: could not retrieve the list from pack %s",
            assetPackName);
    }
  }
  ...
  void LoadTexturesFromAssetPack(const char *assetPackName) {
    ...
    if (assetPackFiles != NULL) {
      for (int i = 0; i < assetPackFileCount; ++i) {
        uint64_t fileSize =
            gameAssetManager->GetGameAssetSize(assetPackFiles[i]);
        ALOGI("TextureLoader: the size of asset %s is %d", assetPackFiles[i],
              (int)fileSize);
        if (fileSize > 0) {
          uint8_t *fileBuffer = static_cast<uint8_t *>(malloc(fileSize));
          if (gameAssetManager->LoadGameAssetAsync(
                  assetPackFiles[i], fileSize, fileBuffer, LoadingCallbackProxy,
                  this)) {
      ...
      ALOGI("LoaderScene: could not retrieve the list from pack %s",
            assetPackName);
    }
  ...
  void CreateTextures() {
    TextureManager *textureManager =
        NativeEngine::GetInstance()->GetTextureManager();
    for (int i = 0; i < _currentLoadIndex; ++i) {
      textureManager->CreateTexture(
          _loadedTextures[i].textureName, _loadedTextures[i].textureSize,
...
void LoaderScene::DoFrame() {
  GameAssetManager *gameAssetManager =
      NativeEngine::GetInstance()->GetGameAssetManager();
  if (!mTextureLoader->IsAssetPackInstalled(
          GameAssetManifest::MAIN_ASSETPACK_NAME) &&
      gameAssetManager->GetGameAssetPackStatus(
          GameAssetManifest::MAIN_ASSETPACK_NAME) ==
          GameAssetManager::GAMEASSET_READY) {
    ALOGI("LoaderScene: attempting to install asset pack %s",
          GameAssetManifest::MAIN_ASSETPACK_NAME);
    mTextureLoader->LoadTexturesFromAssetPack(
        GameAssetManifest::MAIN_ASSETPACK_NAME);
  }
  if (gameAssetManager->GetGameAssetPackStatus(
    ...
    mTextureLoader->InstallTexturesFromAssetPack(
        GameAssetManifest::EXPANSION_ASSETPACK_NAME);
  } else if (!mTextureLoader->IsAssetPackInstalled(
                 GameAssetManifest::EXPANSION_ASSETPACK_NAME) &&
             gameAssetManager->GetGameAssetPackStatus(
                 GameAssetManifest::EXPANSION_ASSETPACK_NAME) ==
                 GameAssetManager::GAMEASSET_READY) {
  ...
  if (mTextureLoader->NumberRemainingToLoad() == 0 &&
    ...
    TuningManager *tuningManager =
        NativeEngine::GetInstance()->GetTuningManager();
    tuningManager->FinishLoading();

    timespec currentTimeSpec;
    clock_gettime(CLOCK_MONOTONIC, &currentTimeSpec);
    uint64_t currentTime =
        currentTimeSpec.tv_sec * 1000 + (currentTimeSpec.tv_nsec / 1000000);
    ...
    float totalLoad = mTextureLoader->TotalNumberToLoad() +
                      DATA_LOAD_DELTA * mDataStateMachine->getTotalSteps();
    float completedLoad =
        mTextureLoader->NumberCompetedLoading() +
        DATA_LOAD_DELTA * mDataStateMachine->getStepsCompleted();
...
void LoaderScene::OnCreateWidgets() {
  ...
  mStartTime =
      currentTimeSpec.tv_sec * 1000 + (currentTimeSpec.tv_nsec / 1000000);
  mTextureLoader->FindTexturesFromAssetPack(
      GameAssetManifest::MAIN_ASSETPACK_NAME);
  mTextureLoader->FindTexturesFromAssetPack(
      GameAssetManifest::EXPANSION_ASSETPACK_NAME);
}
