...
class MemoryConsumer {
 public:
  ...
  // Return the number of times memory advice has reported a critical memory
  // warning status
  uint32_t GetCriticalMemoryWarningCount() const {
    return mCriticalMemoryWarningCount;
  }

  // Return the memory state most recently reported by the memory advice
  // library, this value can be cast to MemoryAdvice_MemoryState
  int32_t GetLastReportedMemoryState() const {
    return mLastReportedMemoryState;
  }
 ...
 private:
  ...
  void UpdatePoolAllocations(const MemoryPools memoryPool,
