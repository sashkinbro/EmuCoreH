...
static const size_t ALLOC_SIZES[BUCKET_COUNT] = {
    ...
    (512 * 1024), (768 * 1024), (1024 * 1024), (2048 * 1024)};

// Milliseconds to delay before next alloc
static const uint64_t ALLOC_DELTA_TIME[BUCKET_COUNT] = {10, 20, 30,  40,
                                                        50, 75, 100, 200};
...
// Very basic structure and class to track pools of memory allocations, only
// aggregate allocation count and size, and complete drain of all pool
...
class MemoryConsumerPool {
 public:
  ...
  uint64_t GetTotalBytesAllocated() const { return mTotalBytesAllocated; }

  uint64_t GetTotalAllocationCount() const { return mAllocationCount; }
...
static void MemoryWatcherCallback(MemoryAdvice_MemoryState state,
                                  void *user_data) {
...
MemoryConsumer::MemoryConsumer(bool startAsActive) {
  ...
  MemoryAdvice_ErrorCode error = MemoryAdvice_registerWatcher(
      MEMORY_CALLBACK_PERIOD_MS, MemoryWatcherCallback, this);
...
MemoryConsumer::~MemoryConsumer() {
  MemoryAdvice_ErrorCode error =
      MemoryAdvice_unregisterWatcher(MemoryWatcherCallback);
  if (error != MEMORYADVICE_ERROR_OK) {
...
void MemoryConsumer::Update() {
  if (mActive) {
    ...
    if (currentTimestamp - mPercentageAvailableCheckTimestamp >
        MEMORY_CALLBACK_PERIOD_MS) {
      mPercentageAvailableCheckTimestamp = currentTimestamp;
      mLastReportedMemoryPercentage =
...
void MemoryConsumer::UpdatePoolAllocations(const MemoryPools memoryPool,
  ...
  const uint64_t deltaTime =
      currentTimestamp - mPools[memoryPool]->GetLastAllocationTimestamp();
...
size_t MemoryConsumer::GetTotalPoolAllocatedBytes(
    const MemoryPools memoryPool) const {
...
void MemoryConsumer::RenderMemoryStatistics(TextRenderer *textRenderer) const {
  if (mActive) {
    const size_t generalPoolMB =
        GetTotalPoolAllocatedBytes(MEMORY_POOL_GENERAL) / (1024 * 1024);
    const size_t cachePoolMB =
        GetTotalPoolAllocatedBytes(MEMORY_POOL_CACHE) / (1024 * 1024);
    const int percentAvailable =
        static_cast<int>(mLastReportedMemoryPercentage);
    ...
    snprintf(memoryString, 63, "%s (%d) %d%% G: %zu MB C: %zu MB", statusString,
             mCriticalMemoryWarningCount, percentAvailable, generalPoolMB,
             cachePoolMB);
