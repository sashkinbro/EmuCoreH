...
void PlayScene::OnStartGraphics() {
  ...
  for (int wallIndex = 0; wallIndex < MAX_WALL_TEXTURES; ++wallIndex) {
    ...
    uint64_t textureReference =
        textureManager->GetTextureReference(textureName);
    if (textureReference != TextureManager::INVALID_TEXTURE_REF) {
      uint32_t mipCount = textureManager->GetTextureMipCount(textureName);
      mWallTextures[wallIndex] =
          new Texture(static_cast<GLuint>(textureReference), mipCount);
  ...
  if (mActiveWallTextureCount == 0) {
    ...
    mWallTextures[0]->InitFromRawRGB(WALL_TEXTURE_SIZE, WALL_TEXTURE_SIZE,
                                     false, _gen_wall_texture());
    ++mActiveWallTextureCount;
  }
  ...
  if (mMenu) {
    if (mMenu == MENU_LOADING) {
      ...
      if (dataStateMachine->isLoadingDataCompleted()) {
        ...
        int loadingPercentage = dataStateMachine->getStepsCompleted() * 100 /
                                dataStateMachine->getTotalSteps();
        snprintf(mMenuItemText[MENUITEM_LOADING], 64, "%s... %d%%", S_LOADING,
                 loadingPercentage);
      }
    }
  ...
  if (o->grid[col][row]) {
    ...
    if (mDifficulty < score / SCORE_PER_LEVEL) {
      ...
      if (NativeEngine::GetInstance()->SaveProgress(mDifficulty)) {
        // Show a "checkpoint saved" sign when possible. We don't show it right
        // away because will already be showing the "Level N" sign, so we just
        // set this flag to remind us to show it right after.
...
void PlayScene::HandleMenu(int menuItem) {
  switch (menuItem) {
    ...
    case MENUITEM_RESUME:
      // resume from saved level
      mDifficulty =
          (mSavedLevel / LEVELS_PER_CHECKPOINT) * LEVELS_PER_CHECKPOINT;
    ...
    case MENUITEM_RESUME_CLOUD:
      DataLoaderStateMachine *dataStateMachine =
          NativeEngine::GetInstance()->GetDataStateMachine();
      mSavedLevel =
          (dataStateMachine->getLevelLoaded() / LEVELS_PER_CHECKPOINT) *
