...
static int32_t _checkControllerButton(const uint32_t buttonsDown,
...
bool isDirectionalKey(const int32_t ourKeyCode) {
  return ourKeyCode == OURKEY_UP || ourKeyCode == OURKEY_LEFT ||
         ourKeyCode == OURKEY_DOWN || ourKeyCode == OURKEY_RIGHT;
}
 ...
 *
 * AMOTION_EVENT_ACTION_POINTER_DOWN and AMOTION_EVENT_ACTION_POINTER_UP are
 * sent for secondary (second, third, fourth, etc.) pointer events. The pointer
 * index into the GameActivityMotionEvent.pointers array is obtained by AND
 * masking the GameActivityMotionEvent.action field with
 * AMOTION_EVENT_ACTION_POINTER_INDEX_MASK and then shifting by
 * AMOTION_EVENT_ACTION_POINTER_INDEX_SHIFT. This index is *only* provided on
 * AMOTION_EVENT_ACTION_POINTER_DOWN and AMOTION_EVENT_ACTION_POINTER_UP events.
 *
 * AMOTION_EVENT_ACTION_MOVE includes the current coordinates of *all* currently
 * active pointers, both primary and any secondary pointers. We use the
 * GameActivityMotionEvent.pointerCount field to loop through and process the
 * GameActivityMotionEvent.pointers array. Note that
 * AMOTION_EVENT_ACTION_POINTER_INDEX_MASK is not valid for
 * AMOTION_EVENT_ACTION_MOVE events.
 */
bool CookGameActivityMotionEvent(GameActivityMotionEvent *motionEvent,
  ...
  if (motionEvent->pointerCount > 0) {
    ...
    // event at the end of the function if pointerIndex is set to a valid index
    // range
    uint32_t pointerIndex = GAMEACTIVITY_MAX_NUM_POINTERS_IN_MOTION_EVENT;
    struct CookedEvent ev;
    memset(&ev, 0, sizeof(ev));
    ev.motionIsOnScreen =
        (motionEvent->source & AINPUT_SOURCE_TOUCHSCREEN) != 0;
  ...
  if (gameControllerIndex >= 0) {
    ...
    if (Paddleboat_getControllerData(gameControllerIndex, &controllerData) ==
      ...
      for (int i = 0; i < INPUT_ACTION_COUNT; ++i) {
        addedControllerEvent |= _checkControllerButton(
            controllerData.buttonsDown, _INPUT_ACTIONS[i], callback);
      }
