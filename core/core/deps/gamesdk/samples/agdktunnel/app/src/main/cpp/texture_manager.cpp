...
static TextureFileFormat GetFileFormat(const uint8_t *fileData,
...
static bool CreateFromASTCFile(const uint8_t *fileData, const size_t fileSize,
                               GLuint *textureID) {
  bool success = false;
  const ASTCHeader *header = reinterpret_cast<const ASTCHeader *>(fileData);
  if (header->blockDepth == 1 && header->texDepth[0] == 1) {
    GLsizei textureWidth = header->texWidth[0] | (header->texWidth[1] << 8) |
                           (header->texWidth[2] << 8);
    GLsizei textureHeight = header->texHeight[0] | (header->texHeight[1] << 8) |
                            (header->texHeight[2] << 8);
    GLenum textureFormat = 0;
    const GLsizei textureSize =
        static_cast<const GLsizei>(fileSize - sizeof(ASTCHeader));
    ...
    if (textureFormat > 0) {
      ...
      glCompressedTexImage2D(GL_TEXTURE_2D, 0, textureFormat, textureWidth,
                             textureHeight, 0, textureSize, textureData);
...
// This is not a robust KTX loader, ala libktx. It is only intended to load the
// KTX 1.1 ETC2 format, mip-mapped 2D texture files included with this example
static bool CreateFromKTXFile(const uint8_t *fileData,
                              const size_t /*fileSize*/, GLuint *textureID,
  ...
  const size_t textureOffset =
      (sizeof(KTXHeader) + header->bytesOfKeyValueData + 3) & (~3);
  const uint32_t *textureSize =
      reinterpret_cast<const uint32_t *>(fileData + textureOffset);
  const uint8_t *textureData =
      reinterpret_cast<const uint8_t *>(textureSize + 1);
  ...
  if (header->glInternalFormat >= GL_COMPRESSED_RGB8_ETC2 &&
    ...
    const GLint minFilter =
        header->numberOfMipmapLevels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR;
    // Clear error
    glGetError();
    ...
    for (uint32_t currentMipLevel = 0;
         currentMipLevel < header->numberOfMipmapLevels; ++currentMipLevel) {
      const uint32_t currentMipSize = *textureSize;
      glCompressedTexImage2D(GL_TEXTURE_2D, currentMipLevel,
                             header->glInternalFormat, currentMipWidth,
                             currentMipHeight, 0, currentMipSize, textureData);
      ...
      textureSize =
          reinterpret_cast<const uint32_t *>(textureData + currentMipSize);
...
TextureManager::TextureManager() {
  ...
  for (GLint i = 0; i < extensionCount; i++) {
    ...
    if (strcmp(reinterpret_cast<const char *>(extensionString),
               ASTC_EXTENSION_STRING) == 0) {
      mDeviceSupportsASTC = true;
      break;
    }
  }
  ALOGI("ASTC Textures: %s",
        (mDeviceSupportsASTC ? "Supported" : "Not Supported"));
}
...
bool TextureManager::IsTextureLoaded(const char *textureName) {
  TextureManager::TextureReference textureRef =
      FindReferenceForName(textureName);
...
bool TextureManager::LoadTexture(const char *textureName) {
  ...
  if (!IsTextureLoaded(textureName)) {
    GameAssetManager *gameAssetManager =
        NativeEngine::GetInstance()->GetGameAssetManager();
...
bool TextureManager::CreateTexture(const char *textureName,
  ...
  if (fileFormat == TEXTUREFILE_ASTC && mDeviceSupportsASTC) {
    ...
    success = CreateFromKTXFile(textureData, textureSize, &textureID,
                                &textureMipCount);
    mLastTextureFormat = TEXTUREFORMAT_ETC2;
  } else {
    ALOGE("TextureManager: unknown texture file format in file: %s",
          textureName);
  }

  if (success) {
    mTextures.push_back(TextureManager::TextureReference(
        textureMipCount, textureName, static_cast<uint64_t>(textureID)));
  }
...
TextureManager::TextureReference TextureManager::FindReferenceForName(
