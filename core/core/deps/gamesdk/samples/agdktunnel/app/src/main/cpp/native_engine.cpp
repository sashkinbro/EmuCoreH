...
static void _GameControllerStatusCallback(
  ...
  if (_singleton != NULL) {
    ...
    if (status == PADDLEBOAT_CONTROLLER_JUST_CONNECTED) {
      ...
      if (controllerIndex == _singleton->GetActiveGameControllerIndex()) {
        // Default to no fallback controller, loop and look for another
        // connected one
        int32_t newControllerIndex = -1;

        for (int32_t i = 0; i < PADDLEBOAT_MAX_CONTROLLERS; ++i) {
          if (i != controllerIndex && Paddleboat_getControllerStatus(i) ==
...
NativeEngine::NativeEngine(struct android_app *app) {
  ...
  mGameAssetManager =
      new GameAssetManager(app->activity->assetManager, app->activity->vm,
  ...
  const MemoryAdvice_ErrorCode memoryError =
      MemoryAdvice_init(GetJniEnv(), app->activity->javaGameActivity);
  ...
  mTuningManager = new TuningManager(
      GetJniEnv(), app->activity->javaGameActivity, app->config);
   ...
   * will only allow user to input numbers. Don't forget IME_FLAG_NO_FULLSCREEN
   * flag, otherwise the software keyboard will occupy the whole screen. */
  /* GameActivity_setImeEditorInfo(mApp->activity, TYPE_CLASS_NUMBER,
                                IME_ACTION_NONE, IME_ACTION_DONE |
     IME_FLAG_NO_FULLSCREEN); */

  // Set fields retrieved through JNI
  // Find the Java class
  jclass activityClass =
      GetJniEnv()->GetObjectClass(mApp->activity->javaGameActivity);
  ...
  jboolean isCopy;
  const char *str =
      GetJniEnv()->GetStringUTFChars((jstring)jInternalStoragePath, &isCopy);
  ...
  jmethodID isPlayGamesServicesLinkedID = GetJniEnv()->GetMethodID(
      activityClass, "isPlayGamesServicesLinked", "()Z");
  mCloudSaveEnabled = (bool)GetJniEnv()->CallBooleanMethod(
      mApp->activity->javaGameActivity, isPlayGamesServicesLinkedID);

  mDataStateMachine =
      new DataLoaderStateMachine(mCloudSaveEnabled, internalStoragePath);
}
...
DataLoaderStateMachine *NativeEngine::BeginSavedGameLoad() {
  if (IsCloudSaveEnabled()) {
    ...
    GetJniEnv()->CallVoidMethod(mApp->activity->javaGameActivity,
                                loadCloudCheckpointID);
  } else {
...
bool NativeEngine::SaveProgress(int level, bool forceSave) {
  if (!forceSave) {
    if (level <= mDataStateMachine->getLevelLoaded()) {
      // nothing to do
      ALOGI("No need to save level, current = %d, saved = %d", level,
            mDataStateMachine->getLevelLoaded());
      return false;
    } else if (!IsCheckpointLevel(level)) {
      ALOGI("Current level %d is not a checkpoint level. Nothing to save.",
            level);
      return false;
    }
  }
...
void NativeEngine::SaveGameToCloud(int level) {
  ...
  GetJniEnv()->CallVoidMethod(mApp->activity->javaGameActivity,
                              saveCloudCheckpointID, (jint)level);
}

// TODO: rename the methods according to your package name
extern "C" jboolean
Java_com_google_sample_agdktunnel_PGSManager_isLoadingWorkInProgress(
...
extern "C" void
Java_com_google_sample_agdktunnel_PGSManager_savedStateInitLoading(
    JNIEnv * /*env*/, jobject /*pgsManager*/) {
  NativeEngine *instance = NativeEngine::GetInstance();
...
extern "C" void
Java_com_google_sample_agdktunnel_PGSManager_authenticationCompleted(
    JNIEnv * /*env*/, jobject /*pgsManager*/) {
  NativeEngine *instance = NativeEngine::GetInstance();
...
extern "C" void
Java_com_google_sample_agdktunnel_PGSManager_authenticationFailed(
    JNIEnv * /*env*/, jobject /*pgsManager*/) {
  NativeEngine *instance = NativeEngine::GetInstance();
...
extern "C" void
Java_com_google_sample_agdktunnel_PGSManager_savedStateSnapshotNotFound(
    JNIEnv * /*env*/, jobject /*pgsManager*/) {
  NativeEngine *instance = NativeEngine::GetInstance();
...
extern "C" void
Java_com_google_sample_agdktunnel_PGSManager_savedStateCloudDataFound(
...
extern "C" void
Java_com_google_sample_agdktunnel_PGSManager_savedStateLoadingFailed(
    JNIEnv * /*env*/, jobject /*pgsManager*/) {
  NativeEngine *instance = NativeEngine::GetInstance();
...
extern "C" void
Java_com_google_sample_agdktunnel_PGSManager_savedStateLoadingCompleted(
  ...
  switch (cmd) {
    ...
    case APP_CMD_WINDOW_INSETS_CHANGED:
      ...
      for (int type = 0; type < GAMECOMMON_INSETS_TYPE_COUNT; ++type) {
        GameActivity_getWindowInsets(mApp->activity, (GameCommonInsetsType)type,
                                     &insets);
        VLOGD("%s insets: left=%d right=%d top=%d bottom=%d",
              sInsetsTypeName[type], insets.left, insets.right, insets.top,
              insets.bottom);
      }
      break;
    case APP_CMD_SOFTWARE_KB_VIS_CHANGED:
      ...
      VLOGD("software keyboard visible: %s",
            (GameActivity_isSoftwareKeyboardVisible(mApp->activity) ? "yes"
                                                                    : "no"));
      break;
...
bool NativeEngine::InitContext() {
  ...
  EGLint attribList[] = {EGL_CONTEXT_CLIENT_VERSION, 3,
                         EGL_NONE};  // OpenGL ES 3.0
...
void NativeEngine::SetInputSdkContext(int context) {
  ...
  GetJniEnv()->CallVoidMethod(mApp->activity->javaGameActivity,
                              setInputContextID, (jint)context);
}
