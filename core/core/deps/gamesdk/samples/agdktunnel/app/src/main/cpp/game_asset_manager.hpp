...
class GameAssetManager {
 public:
  enum GameAssetPackType {
    // This asset pack type is always available and included in the application
    // package
    GAMEASSET_PACKTYPE_INTERNAL = 0,
    // This asset pack type is downloaded separately but is an automatic
    // download after
    // app installation
  ...
  enum GameAssetStatus {
    ...
    // The asset pack is waiting for information about its status to become
    // available
    ...
    // The asset pack is large enough to require explicit authorization to
    // download
    ...
    // The asset pack is pending the results of a request for download
    // cancellation,
  ...
  GameAssetManager(AAssetManager *assetManager, JavaVM *jvm,
  ...
  // If the status of the asset is GAMEASSET_READY, load file data into the
  // specified buffer, returns true if successful
  bool LoadGameAsset(const char *assetName, const size_t bufferSize,
                     void *loadBuffer);

  // If the status of the asset is GAMEASSET_READY, start asynchronously loading
  // file data into the specified buffer. Callback will be called when load
  // completes. returns true if async load began successfully. userData is
  // passed without modification to the callback.
  bool LoadGameAssetAsync(const char *assetName, const size_t bufferSize,
  ...
  const char **GetGameAssetPackFileList(const char *assetPackName,
                                        int *fileListSize);
  ...
  // Request permission to download large asset packs over a mobile data
  // connection
  ...
  // Requests a cancellation of the download process for the specified asset
  // pack, is only a request, not a guarantee, will have no impact on an asset
  // pack with a status other than GAMEASSET_DOWNLOADING
  void RequestDownloadCancellation(const char *assetPackName);

  // Requests removal  specified asset pack, only fast-follow and on-demand
  // packs can be removed, removal is a request and not guaranteed. Returns
