...
public class InputSDKRemappingListener implements InputRemappingListener {
  ...
  private void processRemappedAction(InputAction remappedInputAction) {
    ...
    if (version == null || version.isEmpty() || version.equals(InputSDKProvider.INPUTMAP_VERSION)) {
      currentInputAction = getCurrentVersionInputAction(remappedActionId);
    } else {
      Log.i(TAG, "Detected version of user-saved input action defers from current version");
      currentInputAction =
          getCurrentVersionInputActionFromPreviousVersion(remappedActionId, version);
    }
    if (!currentInputAction.isPresent()) {
      Log.e(TAG,
          String.format("can't find remapped input action with id %d and version %s",
    ...
    Log.i(TAG,
        String.format("Found input action with id %d remapped from key %s to key %s",
            remappedActionId, keyCodesToString(originalKeyCodes),
