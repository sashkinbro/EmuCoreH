...
public class PGSManager {
  ...
  private Task<Void> loadSnapshot() {
    ...
    return mSnapshotClient
        .open(mSnapshotName, /* createIfNotFound= */ false, mConflictResolutionPolicy)
  ...
  private Task<SnapshotMetadata> saveSnapshot(int level) {
    // Open Snapshot using its name
    return mSnapshotClient
        .open(mSnapshotName, /* createIfNotFound= */ true, mConflictResolutionPolicy)
        .addOnFailureListener(
            mActivity, e -> Log.e(TAG, "Error while opening Snapshot to save data.", e))
        .continueWithTask(task -> {
          if (!task.isSuccessful()) {
            return Tasks.forResult(null);
          }
