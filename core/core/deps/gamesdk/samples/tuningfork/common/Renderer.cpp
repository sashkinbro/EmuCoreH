...
void Renderer::threadMain() {
  ...
  std::lock_guard<std::mutex> lock(mQueueMutex);
  while (mIsActive) {
    mCondVar.wait(mQueueMutex, [this]() REQUIRES(mQueueMutex) {
...
void Renderer::draw(State* state) {
  ...
  if (swappy_enabled) SwappyGL_recordFrameStart(state->display, state->surface);
  ...
  if (dt <= 100ms) {
    deltaSeconds =
        std::chrono::duration_cast<std::chrono::nanoseconds>(dt).count() / 1e9f;
  }
  state->lastUpdate += dt;
