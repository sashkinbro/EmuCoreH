...
void PNG_Memory_Read(png_structp pngStruct, png_bytep destBuffer,
                     png_size_t bytesToRead) {
  PNG_Memory_Stream *pngStream =
      static_cast<PNG_Memory_Stream *>(png_get_io_ptr(pngStruct));
  if (pngStream != NULL) {
...
TextureAssetHandle createTexture(uint32_t texWidth, uint32_t texHeight,
                                 void *texPixels) {
...
TextureAssetHandle loadTextureAsset_LIBPNG(const char *filename,
              ...
              if (png_get_IHDR(pngStruct, pngInfo, &pngWidth, &pngHeight,
                ...
                if (pngWidth > 0 && pngHeight > 0 && pngDepth == 8 &&
                  ...
                  for (png_uint_32 yIdx = 0; yIdx < pngHeight; ++yIdx) {
                    png_read_row(pngStruct, static_cast<png_bytep>(rowPixels),
                                 NULL);
                    const uint8_t *rowPixel =
                        static_cast<const uint8_t *>(rowPixels);
                    for (png_uint_32 xIndex = 0; xIndex < pngWidth; ++xIndex) {
                      *writePixel++ = *rowPixel++;
                      *writePixel++ = *rowPixel++;
                      *writePixel++ = *rowPixel++;
                      *writePixel++ = *rowPixel++;
                  ...
                  returnHandle = createTexture(pngWidth, pngHeight, texPixels);
...
TextureAssetHandle TextureAssetLoader::loadTextureAsset(
    const char *filename, uint32_t *textureWidth, uint32_t *textureHeight) {
