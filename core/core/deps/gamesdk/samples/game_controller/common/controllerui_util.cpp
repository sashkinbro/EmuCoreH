...
ImVec2 GetScaledBasePos(const ControllerUIPanelParams &panelParams,
                        const ImVec2 &basePos) {
  return ImVec2(
      (basePos.x * panelParams.panelImageScale) + panelParams.panelBaseX,
      (basePos.y * panelParams.panelImageScale) + panelParams.panelBaseY);
}

ImVec2 GetScaledTextureSize(const ControllerUIPanelParams &panelParams,
                            const UITextureInfo &texInfo) {
  return ImVec2(
      static_cast<float>(texInfo.textureWidth) * panelParams.panelImageScale,
...
void ControllerUIUtil::Button(const ControllerUIPanelParams &panelParams,
  ...
  const UITextureInfo &buttonTextures =
      ControllerUIData::getUIButtonTextures(buttonId);
  if (buttonTextures.textureHandles[buttonInfo.buttonState] !=
      TextureAssetLoader::INVALID_TEXTURE) {
    const ImVec2 buttonBasePos =
        GetScaledBasePos(panelParams, buttonInfo.basePosition);
    const ImVec2 buttonTextureSize =
        GetScaledTextureSize(panelParams, buttonTextures);
    ImGui::SetCursorPos(CalcPosFromCenter(buttonBasePos, buttonTextureSize));
    ImGui::Image(reinterpret_cast<ImTextureID>(
                     buttonTextures.textureHandles[buttonInfo.buttonState]),
                 buttonTextureSize);
  }
}
...
void ControllerUIUtil::Thumbstick(const ControllerUIPanelParams &panelParams,
  ...
  const UITextureInfo &stickRegionTexture =
      ControllerUIData::getUIStickRegionTexture();
  if (stickRegionTexture.textureHandles[0] !=
      TextureAssetLoader::INVALID_TEXTURE) {
    const ImVec2 stickRegionBasePos = GetScaledBasePos(panelParams, basePos);
    const ImVec2 stickRegionTextureSize =
        GetScaledTextureSize(panelParams, stickRegionTexture);
    ImGui::SetCursorPos(
        CalcPosFromCenter(stickRegionBasePos, stickRegionTextureSize));
    ImGui::Image(
        reinterpret_cast<ImTextureID>(stickRegionTexture.textureHandles[0]),
        stickRegionTextureSize);
  }

  const UITextureInfo stickTextures = ControllerUIData::getUIStickTextures();
  if (stickTextures.textureHandles[stickState] !=
      TextureAssetLoader::INVALID_TEXTURE) {
    const ImVec2 stickTextureSize =
        GetScaledTextureSize(panelParams, stickTextures);
    const ImVec2 adjustedStickPos = GetScaledBasePos(
        panelParams, ImVec2(basePos.x + stickVals.x, basePos.y + stickVals.y));
    ImGui::SetCursorPos(CalcPosFromCenter(adjustedStickPos, stickTextureSize));
    ImGui::Image(
        reinterpret_cast<ImTextureID>(stickTextures.textureHandles[stickState]),
        stickTextureSize);
  }
}
