...
char* readFileLine(FILE* file) {
    ...
    int resultLen =
        stream_util::getline(&mallocBuffer, &mallocBufferSize, file);
...
bool readCpuExtensions(ProtoDataHolder& dataHolder) {
    ...
    while (true) {
        ...
        if (::string_util::startsWith(line.get(), FIELD_KEY)) {
            ...
            // Skip space, tab and colon.
            value = ::string_util::skipChars(value, " \t:");
            // It is safe to cast away const, because it will be deleted right
            // after.
...
int getSystemProp(const char* key, String& result, StringVector& errors) {
    ...
    if (bufferLen > PROP_VALUE_MAX) {
        char error[1024];
        snprintf(error, sizeof(error), "__system_property_get: Overflow: %s",
                 key);
...
int setupEGl(EGLDisplay& display, EGLContext& context, EGLSurface& surface,
    ...
    EGLint pbufferAttribs[] = {
        EGL_WIDTH, androidgamesdk_deviceinfo::OPENGL_TEST_VIEW_WIDTH,
        EGL_HEIGHT, androidgamesdk_deviceinfo::OPENGL_TEST_VIEW_HEIGHT,
        EGL_NONE};
...
void addOglConstsV2_0(androidgamesdk_deviceinfo_GameSdkDeviceInfo_OpenGl& proto,
    ...
    proto.gl_max_renderbuffer_size =
        ::ogl_util::getInt(GL_MAX_RENDERBUFFER_SIZE);
    ...
    GLint numShaderBinFormats =
        ::ogl_util::getInt(GL_NUM_SHADER_BINARY_FORMATS);
    ...
    glGetShaderPrecisionFormat(GL_FRAGMENT_SHADER, GL_MEDIUM_FLOAT, spfr,
                               &spfp);
...
void addOglConstsV3_0(androidgamesdk_deviceinfo_GameSdkDeviceInfo_OpenGl& proto,
    ...
    proto.gl_max_color_attachments =
        ::ogl_util::getInt(GL_MAX_COLOR_ATTACHMENTS);
    ...
    proto.gl_max_elements_vertices =
        ::ogl_util::getInt(GL_MAX_ELEMENTS_VERTICES);
    ...
    proto.gl_max_texture_lod_bias =
        ::ogl_util::getFloat(GL_MAX_TEXTURE_LOD_BIAS);
...
void addOglConstsV3_1(
    androidgamesdk_deviceinfo_GameSdkDeviceInfo_OpenGl& proto) {
    ...
    proto.gl_max_framebuffer_width =
        ::ogl_util::getInt(GL_MAX_FRAMEBUFFER_WIDTH);
    ...
    proto.gl_max_sample_mask_words =
        ::ogl_util::getInt(GL_MAX_SAMPLE_MASK_WORDS);
    ...
    proto.gl_max_uniform_locations =
        ::ogl_util::getInt(GL_MAX_UNIFORM_LOCATIONS);
...
int addGl(androidgamesdk_deviceinfo_GameSdkDeviceInfo_OpenGl& proto,
    ...
    if (glVerMajor >= 3) {
        ...
        ::string_util::splitAddUnique(exts.data.get(), " ",
                                      dataHolder.extension);
    }
...
bool protoEncodeCpuFreqs(pb_ostream_t* stream, const pb_field_t* field,
    ...
    for (size_t i = 0; i < cpuFreqs.size; i++) {
        ...
        if (!pb_encode_submessage(
                stream,
                androidgamesdk_deviceinfo_GameSdkDeviceInfo_CpuCore_fields,
                &core))
            return false;
    }
    return true;
...
bool testCompressedTextureRendering(StringVector& errors, EGLDisplay eglDisplay,
    ...
    if (!basicTextureRenderer.loadTexture(config->internalformat, config->width,
                                          ...
                                          config->data)) {
        errors.addCopy(
            "testCompressedTextureRendering: Unable to load texture.");
        return false;
    }
    ...
    if (flushGlErrors("testCompressedTextureRendering (glReadPixels)",
                      errors) != 0) {
        return false;
    }
    ...
    for (size_t i = 0; i < OPENGL_TEST_VIEW_WIDTH * OPENGL_TEST_VIEW_HEIGHT;
        ...
        if (abs(pixelsData[i].r - config->expectedPixelsColor.r) >
            ...
            snprintf(renderingError, sizeof(renderingError),
                     ...
                     "Got: %d;%d;%d.",
                     i, config->internalformat, pixelsData[i].r,
                     pixelsData[i].g, pixelsData[i].b);
...
void addGlCompressedTexturesRendering(
    ...
    if (openGlDataHolder.compTexFormats.includes(
        ...
        rendered_compressed_texture_formats
            .has_gl_compressed_rgba_astc_4x4_khr = true;
    ...
    if (openGlDataHolder.compTexFormats.includes(
        ...
        rendered_compressed_texture_formats
            .has_gl_compressed_rgba_astc_6x6_khr = true;
    ...
    if (openGlDataHolder.compTexFormats.includes(
        ...
        rendered_compressed_texture_formats
            .has_gl_compressed_rgba_astc_8x8_khr = true;
    ...
    if (openGlDataHolder.compTexFormats.includes(
        ...
        rendered_compressed_texture_formats
            .has_gl_compressed_rgb_pvrtc_2bppv1_img = true;
        rendered_compressed_texture_formats.gl_compressed_rgb_pvrtc_2bppv1_img =
            testCompressedTextureRendering(
                errors, eglDisplay, eglSurface,
                getCompressedTextureTestCase(
                    GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG));
    }
    if (openGlDataHolder.compTexFormats.includes(
    ...
    if (openGlDataHolder.compTexFormats.includes(
        ...
        rendered_compressed_texture_formats
            .has_gl_compressed_rgb_pvrtc_4bppv1_img = true;
    ...
    if (openGlDataHolder.compTexFormats.includes(
        ...
        rendered_compressed_texture_formats
            .has_gl_compressed_rgba_s3tc_dxt1_ext = true;
    ...
    if (openGlDataHolder.compTexFormats.includes(
        ...
        rendered_compressed_texture_formats
            .has_gl_compressed_rgba_s3tc_dxt5_ext = true;
...
int createProto(androidgamesdk_deviceinfo_GameSdkDeviceInfoWithErrors& proto,
    ...
    if (readCpuIndexMax(dataHolder.cpuIndexMax)) {
        ...
        for (int cpuIndex = 0; cpuIndex <= dataHolder.cpuIndexMax; cpuIndex++) {
            ...
            if (!readCpuFreqMax(cpuIndex, dataHolder.cpuFreqs.data[cpuIndex])) {
                // Don't mark a missing cpu frequency as an error, as there
                // might be CPUs with non sequential indexes. The frequency will
                // stay to 0 and omitted when encoded to the proto.
            }
        }
    ...
    if (numErrorsEgl == 0) {
        ...
        androidgamesdk_deviceinfo_GameSdkDeviceInfo_OpenGl& protoGl =
            info.open_gl;
        ...
        protoGl.gl_compressed_texture_formats.arg =
            &dataHolder.ogl.compTexFormats;
        ...
        protoGl.gl_program_binary_formats.funcs.encode =
            &protoEncodeInt32Vector;
