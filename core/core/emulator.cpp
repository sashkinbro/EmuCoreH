...
void Emulator::init()
{
	// A libretro frontend may initialize and deinitialize the core several times
	// in one process (renderer restarts, launching another game). retro_deinit
	// leaves the emulator in the Terminated state, which must be re-armed for
	// the next boot instead of asserting.
