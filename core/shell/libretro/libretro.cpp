...
{
	// gl_ctx_resetting tracks a GL hardware context reset and is only cleared
	// by the GL context_reset(). Tearing down a GL session leaves it set, which
	// must not disable input for the next session running another renderer
	// (Vulkan in particular never goes through the GL context_reset path).
	if (gl_ctx_resetting && isOpenGL(config::RendererType))
		return;
