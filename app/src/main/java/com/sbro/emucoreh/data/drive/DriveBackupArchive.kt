...
import com.sbro.emucoreh.data.ProfileDeviceInfoProvider
...
class DriveBackupArchive(private val context: Context) {
    ...
    private fun roots(): Map<String, File> {
        ...
        return linkedMapOf(
            ...
            "textures" to EmulatorStorage.texturesDir(context)
        )
    }
    ...
    private fun allowed(name: String): Boolean {
        ...
        return when (parts[0]) {
            "memory-cards" -> parts.size == 2 && parts[1].endsWith(".bin", true)
            "save-states" -> parts.size == 2 &&
                (name.endsWith(".rstate", true) || name.endsWith(".png", true))
