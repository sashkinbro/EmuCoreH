...
object EmuCoreHAchievementCatalog {
    const val VERSION = 2
    ...
    val definitions = buildList {
        ...
        EmuAchievementDefinition("shenmue_secret", R.string.achievement_shenmue_title, R.string.achievement_shenmue_description, EmuAchievementMetric.ShenmueMinutes, 120, 50, hidden = true),
        EmuAchievementDefinition("sonic_adventure_secret", R.string.achievement_sonic_adventure_title, R.string.achievement_sonic_adventure_description, EmuAchievementMetric.SonicAdventureMinutes, 120, 50, hidden = true),
        EmuAchievementDefinition("jet_set_radio_secret", R.string.achievement_jet_set_radio_title, R.string.achievement_jet_set_radio_description, EmuAchievementMetric.JetSetRadioMinutes, 90, 50, hidden = true)
        ))
        ...
        val topGames = listOf(
            "Sonic Adventure" to "sonic adventure", "Shenmue" to "shenmue",
            "Jet Set Radio" to "jet set radio|jet grind radio", "Soulcalibur" to "soulcalibur",
            "Crazy Taxi" to "crazy taxi", "Skies of Arcadia" to "skies of arcadia",
            "Phantasy Star Online" to "phantasy star online", "Resident Evil - Code: Veronica" to "code veronica",
            "Marvel vs. Capcom 2" to "marvel vs capcom", "Power Stone" to "power stone",
            "Dead or Alive 2" to "dead or alive 2", "Grandia II" to "grandia ii",
            "Virtua Tennis" to "virtua tennis", "Metropolis Street Racer" to "metropolis street racer"
        )
        topGames.forEachIndexed { index, (game, term) ->
            val slug = term.substringBefore('|').replace(Regex("[^a-z0-9]+"), "_").trim('_')
            add(EmuAchievementDefinition("game_${slug}_time", R.string.achievement_game_time_title, R.string.achievement_game_time_description, EmuAchievementMetric.MatchingTitleMinutes, 120, 50 + index, hidden = true, textArgument = game, matchTerms = term.split('|'), templatedText = true))
            add(EmuAchievementDefinition("game_${slug}_sessions", R.string.achievement_game_sessions_title, R.string.achievement_game_sessions_description, EmuAchievementMetric.MatchingTitleSessions, 10, 45 + index, hidden = true, textArgument = game, matchTerms = term.split('|'), templatedText = true))
        }

        listOf(
            "Sonic" to "sonic", "Shenmue" to "shenmue",
            "Virtua Fighter" to "virtua fighter", "Resident Evil" to "resident evil",
            "Street Fighter" to "street fighter", "Marvel vs. Capcom" to "marvel vs",
            "Phantasy Star" to "phantasy star", "Crazy Taxi" to "crazy taxi"
        ).forEach { (series, term) ->
...
class EmuAchievementRepository(context: Context) {
    ...
    private suspend fun evaluateAndSync(snapshot: AchievementSnapshot): List<EmuAchievementState> {
        ...
        val candidates = EmuCoreHAchievementCatalog.definitions.filter { definition ->
        ...
        val newlyUnlocked = if (candidates.isEmpty()) emptyList() else runCatching { firestore.runTransaction { transaction ->
            ...
            unreadUnlocks.filterNot(AchievementSyncCandidate::unlockExists).map { candidate ->
                ...
                transaction.set(candidate.unlockReference, mapOf(
                    ...
                    "catalogVersion" to EmuCoreHAchievementCatalog.VERSION,
        ...
        val states = EmuCoreHAchievementCatalog.definitions.map { definition ->
        ...
        runCatching {
            firestore.collection(PUBLIC_PROFILES).document(uid).set(
                mapOf(
                    ...
                    "achievementCatalogVersion" to EmuCoreHAchievementCatalog.VERSION,
    ...
    private suspend fun loadPublic(uid: String, profile: PlayerProfile): List<EmuAchievementState> {
        ...
        return EmuCoreHAchievementCatalog.definitions.map { definition ->
            EmuAchievementState(
                definition = definition,
    ...
    private data class AchievementSnapshot(
        ...
        fun progressFor(definition: EmuAchievementDefinition): Long = when (definition.metric) {
            ...
            EmuAchievementMetric.ShenmueMinutes -> minutesForTitle("shenmue")
            EmuAchievementMetric.SonicAdventureMinutes -> minutesForTitle("sonic adventure")
            EmuAchievementMetric.JetSetRadioMinutes -> minutesForTitles(listOf("jet set radio", "jet grind radio"))
        }

        private fun arcadeGames() = games.filter { it.arcade || it.serial.orEmpty().uppercase(Locale.ROOT).let { serial ->
            serial.startsWith("NM") || serial.startsWith("AW")
        } }
        ...
        private fun minutesForTitle(needle: String): Long = minutesForTitles(listOf(needle))

        private fun minutesForTitles(needles: List<String>): Long = games
            .filter { game -> needles.any { needle -> game.title.lowercase(Locale.ROOT).contains(needle) } }
    ...
    private fun PlayerProfile.toAchievementSnapshot() = AchievementSnapshot(
        ...
        games = games.map { AchievementGame(it.title, it.serial, it.totalPlayTimeMs, it.sessions.toLong(), it.arcade) },
    ...
    private fun DocumentSnapshot.toAchievementSnapshot(): AchievementSnapshot? {
        ...
        val games = rawGames.values.mapNotNull { raw ->
            ...
            AchievementGame(
                title = map["t"] as? String ?: return@mapNotNull null,
                serial = map["s"] as? String,
                totalPlayTimeMs = (map["ms"] as? Number)?.toLong() ?: 0L,
                sessions = (map["n"] as? Number)?.toLong() ?: 0L,
                arcade = map["aw"] as? Boolean ?: false
            )
        }
    ...
    private companion object {
        ...
        const val FEED = "feed"
        const val CHANNEL_ID = "emucoreh_achievements"
